...
{{< alert title="Warning" color="warning" >}}
Manually editing generated Prow job YAML is deprecated. These fields should now be configured in the `ci-operator` configuration file instead. See the [ci-operator documentation](/architecture/ci-operator/) for the available fields (`always_run`, `run_if_changed`, `skip_if_only_changed`, `optional`, `max_concurrency`, `reporter_config`, etc.) and the [Prowgen Configuration](/architecture/ci-operator/#prowgen-configuration) section for the `.config.prowgen` migration guide.
{{< /alert >}}
