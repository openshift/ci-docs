...
Jobs in the blocking section will reject the payload if they fail. Informing jobs will not. We need to be careful with what goes into blocking for this reason. See [Extending OpenShift Release Gates](https://docs.ci.openshift.org/architecture/release-gating/) for more on how a job can become blocking.
