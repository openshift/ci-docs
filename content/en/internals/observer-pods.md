...
The `timeout` and the `grace_period` work the same as for [regular steps](/architecture/timeouts/#step-registry-test-process-timeouts).
...
It's a good habit to write the observer payload following this approach, see [here](/architecture/timeouts/#forwarding-signals-to-children-in-steps) for more details on how to trap signals:
{{< highlight bash >}}
#!/bin/bash
