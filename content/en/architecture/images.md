...
CI images are built [as configured on the repository for `ci-operator`](/architecture/ci-operator/#building-container-images). Take `cluster-etcd-operator` (branch `release-4.6`) for an example:
...
Moreover, a developer can build the same container image locally as well if the image build tool such as `docker` or `podman` has [permission to pull the images](/how-tos/use-registries-in-build-farm/#how-do-i-gain-access-to-qci) used in the build manifest. The images in `ocp` namespace are open to all authenticated users.

Golang Build Images
-------------------

There are two types Golang build images created and maintained by ART, they are based on [UBI](https://catalog.redhat.com/en/software/base-images).

### Builder (base)

These images are used for simpler/cleaner builds, as we can see on its own [Dockerfile](https://github.com/openshift-eng/ocp-build-data/blob/openshift-4.22/ci_images/rhel-9/ci-openshift-golang-builder/Dockerfile), they are more than 1GiB lighter than the other ones:
...
These images are the most common used as CI `build-root`, as we can see on [Dockerfile](https://github.com/openshift-eng/ocp-build-data/blob/openshift-4.22/ci_images/rhel-9/ci-openshift-build-root/Dockerfile) they have several tools installed that people came to expect over time like certificates, gcc, some useful binaries and some common Golang modules.
...
Like many other images, these images are mirrored to app.ci's registry and also to `quay.io/openshift/ci`, using `quay.io/openshift/ci:<ns>_<image>_<tag>` convention:

- `quay.io/openshift/ci:ocp_builder_rhel-9-golang-1.25-openshift-4.22`

Although this is not how users would recognize them, their Dockerfiles will mention locations on registry.ci.openshift.org and the sha256 pullspecs will resolve successfully when CI swaps it from `registry.ci.openshift.org` to `quay.io/openshift/ci` (or `quay-proxy`).
