...
that have small variations between them. For example, the following example step declares that it requires a parameter `TEST_SUITE`:
...
A test or workflow that utilizes this step must give a value to the `TEST_SUITE` parameter, which will be available as an environment
