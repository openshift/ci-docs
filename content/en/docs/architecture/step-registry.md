...
Steps and chains can declare parameters they expect to consume in their `env` section. These can then be set to different values to generate tests
that have small variations between them. For example, the following example step declares that it requires a parameter `OPENSHIFT_TEST_SUITE`:
...
A test or workflow that utilizes this step must give a value to the `OPENSHIFT_TEST_SUITE` parameter, which will be available as an environment
...
* `default` (optional): the value assigned if no other node in the hierarchy
  provides one (described in the [hierarchical propagation](#hierarchical-propagation)
  section).
