...
### Configure token for `config-updater`
Ensure `config-updater` SA exists in the `ci` namespace.
```bash
[fedora@Bear-Work-RedHat release]$ oc -n ci get sa config-updater
NAME             SECRETS   AGE
config-updater   1         6m52s
[fedora@Bear-Work-RedHat release]$
```

Ensure you have the domain of the cluster. e.g., `build09.ci.devcluster.openshift.com`.

Generate the token for `config-updater` SA.

```bash
oc -n ci create token config-updater --duration=2592000s
```

Login with the token.
```bash
KUBECONFIG=config-updater.kube oc login https://api.<domain of cluster>:6443 --token=<token>
```

Edit the `vault` item `https://vault.ci.openshift.org/ui/vault/secrets/kv/show/dptp/config-updater` to create a new entry `sa.config-updater.<cluster name>.config`. You might need to rename the context manually.
