...
At this moment `build02` is the only cluster supporting it, but from the test owner standpoint, it will be sufficient to request the `nested-podman` **capability** (see [capabilities](../capabilities/)) on a test.
What follows is an example on how to set the capability on a test:
